...
export const getMarketTransfers = async ({
  player_name,
  team_id,
  price,
  limit,
  ...
  const playerIds = await db
    ...
    .from(players)
    .where(like(players.player_name, `${player_name}%`));
...
export const canAddToMarket = async (teamId: number) => {
  const playerInTransferMarket = await db.$count(
    transferMarket,
    eq(transferMarket.outTeamId, teamId)
  );
...
export const addPlayerToMarker = async ({
  outTeamId,
  playerId,
  price,
}: {
...
export const removePlayerFromMarket = async (
  ...
  await db
    .delete(transferMarket)
    .where(
      and(
        eq(transferMarket.playerId, playerId),
        eq(transferMarket.outTeamId, outTeamId)
      )
    );
};
...
export const buyPlayer = async ({
  ...
  await db.transaction(async (tx) => {
    ...
    await tx
      .update(teams)
      .set({ budget: sql`budget - ${priceToBuy}` })
      .where(eq(teams.id, inTeamId));
    await tx
      .update(teams)
      .set({ budget: sql`budget + ${priceToBuy}` })
      .where(eq(teams.id, outTeamId));
    // add in team to player transfer market
    await tx
      .update(transferMarket)
      .set({ inTeamId })
      .where(eq(transferMarket.playerId, playerId));
  });
};
...
export const getTeamBudget = async (teamId: number) => {
  const result = await db
