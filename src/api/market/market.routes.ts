...
marketRouter.post('/add', async (req, res) => {
    try {
        ...
        if(!canAddToMarket(outTeamId)){
            res.status(400).json({ message: "Can't put players in transfer marker, you should have at least 15 players in your team" });
            return;
        }
        await addPlayerToMarker({ outTeamId, playerId, price });
        res.status(201).json({ message: 'Player has been added to transfer market successfully' });
    } catch (error) {
        console.log("marketRouter.add ~ error:", error)
        res.status(400).json({ message: "An error occurred while adding the player in the transfer market" })
    }
});
...
marketRouter.post('/remove', async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Player has been removed from the transfer market successfully' })
    } catch (error) {
...
marketRouter.post('/buy', async (req, res) => {
    try {
        ...
        if(teamPlayersCount === 25){
            res.status(400).json({ message: "Your team is full, can't have more than 25 players" });
            return;
        }
