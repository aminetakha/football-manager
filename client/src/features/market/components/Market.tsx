...
const Market: React.FC<{ market: MarketType[], userTeamId: number; onBuyPlayerHandler: (data: { playerId: number; outTeamId: number; price: number }) => void; }> = ({ market, userTeamId, onBuyPlayerHandler }) => {
    ...
    const rows = market.map(transfer => (
        <Table.Tr key={transfer.id}>
            <Table.Td>{transfer.player_name} ({transfer.player_position})</Table.Td>
            <Table.Td>
                <Flex align='center' gap='xs'>
                    <Image src={`${apiUrl}/logos/${transfer.out_team_badge}`} w={25} h={25} />
            ...
            <Table.Td><NumberFormatter value={transfer.price || '-'} thousandSeparator prefix={transfer.price? '$' : undefined} /></Table.Td>
